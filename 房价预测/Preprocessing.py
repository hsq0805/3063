...
def main():
    ...
    all_data.drop(['year_built', 'sale_year', 'sale_month', 'sale_day', 'year_repair'], axis=1, inplace=True)

    print(all_data.info())

    lm = LinearRegression()
    df = all_data[0:10000]
    features = ['num_bedroom', 'num_bathroom', 'area_house', 'area_parking',
                'floorage', 'area_basement', 'latitude', 'longitude',
                ...
                'Month_9', 'Month_10', 'Month_11', 'Month_12', 'building_age',
                'have_basement', 'is_repaired', 'room_ratio', 'f_c_ratio', 'f_p_ratio']
    y = df['price']
    ...
    best_score = -1e+12
    '''
    交叉检验的评分标准选择'neg_mean_squared_error'，即均方误差，在这里为负数，绝对值越小表示误差越小，因此初始的best_score设置为一个绝对值很大的负值
    '''
    ...
    # train_X = train_data[
    #     ['area_house', 'latitude', 'building_age', 'num_bedroom', 'num_bathroom', 'room_ratio', 'f_c_ratio',
    #      'longitude', 'have_basement', 'floorage', 'Month_4', 'Year_(2005, 2020]', 'is_repaired', 'Month_3',
    #      'Month_12', 'Month_9', 'Year_(1975, 1990]']]
    # train_Y = train_data["price"]
    # test_X = test_data[
    #     ['area_house', 'latitude', 'building_age', 'num_bedroom', 'num_bathroom', 'room_ratio', 'f_c_ratio',
    #      'longitude', 'have_basement', 'floorage', 'Month_4', 'Year_(2005, 2020]', 'is_repaired', 'Month_3',
